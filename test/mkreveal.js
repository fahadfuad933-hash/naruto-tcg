  ...
  setTimeout(function () {
    try {
      ...
      const clickIv = setInterval(function () {
        ...
        const pbtn = $('#phase-btn');
        if (pbtn && pbtn.disabled && summoned) return; // UI beschäftigt (Animation) → warten
        if (!summoned) {
      ...
      setTimeout(function () { clearInterval(poll); finish(); }, 20000);
    } catch (e) {
...
fs.writeFileSync(path.join(__dirname, 'reveal.gen.html'), out);
